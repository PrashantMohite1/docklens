...
B) docker image verify image-name --local file.txt:/app/file.txt
C) docker image verify img-name -- local folder:/app/folder
D) docker image verify [img1,img2,img3] --local file.txt:/app/file.txt
